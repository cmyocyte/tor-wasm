...
use tor_wasm::network::{WasmTcpProvider, WasmTlsConnector, NetworkConfig, ConnectionManager};
...
async fn test_connect_to_tor_directory() {
    ...
    match provider.connect_with_retry(&addr).await {
        Ok(mut stream) => {
            ...
            web_sys::console::log_1(&format!("📄 Response preview: {}", &response_str[..200.min(n)]).into());
...
async fn test_concurrent_connections() {
    ...
    for addr in addresses {
        match manager.get_connection(&addr).await {
            ...
            Err(e) => {
                web_sys::console::error_1(&format!("❌ Failed to connect to {}: {}", addr, e).into());
            }
        }
    }
    
    let stats = provider.get_stats();
    web_sys::console::log_1(&format!("📊 Stats: {} attempted, {} successful", 
        stats.connections_attempted, stats.connections_successful).into());
...
async fn test_connection_retry() {
    ...
    match provider.connect_with_retry(&addr).await {
        ...
        Err(e) => {
            ...
            web_sys::console::log_1(&format!("✅ Correctly failed after {} ms: {}", elapsed, e).into());
