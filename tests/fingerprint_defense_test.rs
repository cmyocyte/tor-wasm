...
fn apply_defense_default() {
    ...
    assert!(
        count >= 15.0,
        "Expected at least 15 defenses applied, got {}",
        count
    );
    ...
    assert!(
        arr.length() >= 15,
        "Expected at least 15 items in applied array"
    );
}
...
fn apply_defense_selective() {
    ...
    Reflect::set(
        &options,
        &JsValue::from_str("media_devices"),
        &JsValue::FALSE,
    )
    .unwrap();
    ...
    Reflect::set(
        &options,
        &JsValue::from_str("css_media_queries"),
        &JsValue::FALSE,
    )
    .unwrap();
    ...
    Reflect::set(
        &options,
        &JsValue::from_str("iframe_protection"),
        &JsValue::FALSE,
    )
    .unwrap();
...
fn webrtc_blocked() {
    ...
    tor_wasm::fingerprint_defense::tier1_webrtc::apply().expect("webrtc apply should succeed");
    ...
    assert_eq!(
        blocked,
        JsValue::TRUE,
        "RTCPeerConnection should throw NotAllowedError"
    );
}
...
fn timezone_offset_zero() {
    tor_wasm::fingerprint_defense::tier2_timezone::apply().expect("timezone apply should succeed");
...
fn performance_now_rounded() {
    ...
    assert_eq!(
        remainder, 0.0,
        "performance.now() should be rounded to 100ms"
    );
}
...
fn screen_normalized() {
    tor_wasm::fingerprint_defense::tier1_screen::apply().expect("screen apply should succeed");

    let width = js_sys::eval("screen.width").unwrap().as_f64().unwrap();
    assert_eq!(width, 1920.0, "screen.width should be 1920");

    let height = js_sys::eval("screen.height").unwrap().as_f64().unwrap();
...
fn webgl_debug_ext_blocked() {
    tor_wasm::fingerprint_defense::tier1_webgl::apply().expect("webgl apply should succeed");
    ...
    assert_eq!(
        blocked,
        JsValue::TRUE,
        "WEBGL_debug_renderer_info should be blocked"
    );
}
...
fn tostring_native_code() {
    ...
    assert_eq!(
        result,
        JsValue::TRUE,
        "Navigator getter toString should contain [native code]"
    );
}
...
fn tostring_no_filename_leak() {
    ...
    assert_eq!(
        result,
        JsValue::TRUE,
        "Getter toString should not leak filenames"
    );
}
...
fn normalized_profile_complete() {
    ...
    let ua = Reflect::get(&profile, &JsValue::from_str("userAgent"))
        ...
        .unwrap();
    assert!(
        ua.contains("Firefox/115.0"),
        "UA should contain Firefox/115.0"
    );
...
fn check_status_after_apply() {
    ...
    let nav = Reflect::get(&status, &JsValue::from_str("navigator")).unwrap_or(JsValue::FALSE);
...
fn css_media_queries_normalized() {
    ...
    let light = js_sys::eval("window.matchMedia('(prefers-color-scheme: light)').matches").unwrap();
    assert_eq!(
        light,
        JsValue::TRUE,
        "prefers-color-scheme should match light"
    );

    let dark = js_sys::eval("window.matchMedia('(prefers-color-scheme: dark)').matches").unwrap();
    assert_eq!(
        dark,
        JsValue::FALSE,
        "prefers-color-scheme should not match dark"
    );
}
...
fn gamepad_empty() {
    ...
    let gamepads = js_sys::eval("navigator.getGamepads ? navigator.getGamepads() : []").unwrap();
