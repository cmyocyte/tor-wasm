...
impl GuardState {
    ...
    pub fn needs_refresh(&self) -> bool {
        ...
        if usable_guards < MIN_GUARDS {
            log::info!("🔄 Too few usable guards ({}/{})", usable_guards, MIN_GUARDS);
            return true;
        }
    ...
    pub fn select_guards(&mut self, relays: &[Relay]) -> Result<()> {
        ...
        let mut guard_candidates: Vec<_> = relays.iter()
            .filter(|r| {
                r.is_guard() && 
                r.is_running() && 
                r.is_stable() &&
        ...
        log::info!("🛡️ Selected {} guards, valid until {}", 
            self.guards.len(),
    ...
    fn mark_bad(&mut self, fingerprint: &str) {
        ...
        log::warn!("🚫 Guard {} marked as bad until {}", 
            &fingerprint[..8.min(fingerprint.len())],
