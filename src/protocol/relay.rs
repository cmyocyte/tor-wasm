...
impl RelaySelector {
    ...
    pub fn select_guards(&self, count: usize) -> Vec<&Relay> {
        ...
        if !self.preferred_guards.is_empty() {
            log::info!(
                "🛡️ Prioritizing {} preferred guards",
                self.preferred_guards.len()
            );
            ...
            for preferred_fp in &self.preferred_guards {
                ...
                if let Some(relay) = self.relays.iter().find(|r| {
                    ...
                    log::warn!(
                        "  ⚠️ Preferred guard {} not found in consensus",
        ...
        log::info!(
            "🎲 Selected {} guards ({} preferred, {} fallback)",
            selected.len(),
            preferred_count,
    ...
    pub fn select_middles(&self, count: usize, exclude: &[&str]) -> Vec<&Relay> {
        ...
        let mut middles: Vec<&Relay> = self
            .relays
            .iter()
            .filter(|r| {
                r.is_middle()
    ...
    pub fn guards(&self) -> Vec<&Relay> {
        self.relays.iter().filter(|r| r.is_guard()).collect()
    }

    /// Get all exit relays
    pub fn exits(&self) -> Vec<&Relay> {
        self.relays.iter().filter(|r| r.is_exit()).collect()
    }
