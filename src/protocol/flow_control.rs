...
impl StreamFlowControl {
    ...
    pub fn on_sendme_received(&mut self) {
        self.send_window += Self::WINDOW_INCREMENT;
        log::debug!("Stream {} send window: {} (+{})", 
            self.stream_id, self.send_window, Self::WINDOW_INCREMENT);
    }
    ...
    pub fn on_receive_data(&mut self) -> bool {
        ...
        if self.recv_window == 0 {
            // Reset window
            self.recv_window = Self::WINDOW_INCREMENT;
            log::debug!("Stream {} recv window depleted, sending SENDME", self.stream_id);
            return true;
        }
