...
impl NtorHandshake {
    ...
    fn validate_entropy(bytes: &[u8]) {
        ...
        if unique_bytes.len() < 8 {
            log::error!("❌ CRITICAL: Key has low entropy ({} unique bytes)!", unique_bytes.len());
            panic!("RNG failure: low entropy");
        }
        
        log::trace!("✅ Entropy validation passed ({} unique bytes)", unique_bytes.len());
    }
    ...
    pub fn complete(
        ...
        let mut mac_verify = HmacSha256::new_from_slice(T_VERIFY)
            .expect("HMAC can take key of any size");
        ...
        let mut mac_auth = HmacSha256::new_from_slice(T_MAC)
            .expect("HMAC can take key of any size");
...
mod tests {
    ...
    fn test_create_handshake_data() {
        ...
        let data = NtorHandshake::create_handshake_data(
            &client_public,
            &relay_fingerprint,
            &relay_onion,
        );
