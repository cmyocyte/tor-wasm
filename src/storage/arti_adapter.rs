...
use super::{WasmStorage, ConsensusData, RelayData, ClientState};
...
impl GuardManager {
    ...
    pub async fn add_guard(&self, guard: Guard) -> Result<()> {
        ...
        if guards.guards.iter().any(|g| g.fingerprint == guard.fingerprint) {
    ...
    pub async fn mark_used(&self, fingerprint: &str) -> Result<()> {
        ...
        if let Some(guard) = guards.guards.iter_mut().find(|g| g.fingerprint == fingerprint) {
    ...
    pub async fn mark_failed(&self, fingerprint: &str) -> Result<()> {
        ...
        if let Some(guard) = guards.guards.iter_mut().find(|g| g.fingerprint == fingerprint) {
            ...
            if guard.failure_count > 3 {
                ...
                log::warn!("Guard {} marked unreachable after {} failures", 
                    fingerprint, guard.failure_count);
            }
        }
