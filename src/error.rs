...
impl TorError {
    ...
    pub fn user_message(&self) -> String {
        match self {
            // Connection
            TorError::ConnectionFailed(_) => 
                "Failed to connect to the Tor network. Please check your internet connection.".into(),
            TorError::Timeout => 
            ...
            TorError::ProtocolError(_) => 
                "A protocol error occurred. Please try again.".into(),
            TorError::UnexpectedCell { .. } => 
                "Received unexpected data from the Tor network.".into(),
            TorError::DigestMismatch => 
                "Data integrity check failed. Please try again.".into(),
            TorError::HandshakeFailed(_) => 
            ...
            TorError::CircuitDestroyed { reason, .. } => 
                format!("Your circuit was closed by a relay (reason {}). Please try again.", reason),
            TorError::AllRelaysFailed => 
                "All available relays failed. Please try again later.".into(),
            TorError::CircuitClosed(_) => 
                "Your circuit was closed. Please try again.".into(),
            TorError::Stream(_) => 
                "Data transfer failed. Please try again.".into(),
            ...
            TorError::InvalidUrl(_) => 
                "Invalid URL provided. Please check the URL format.".into(),
            TorError::ParseError(_) => 
                "Failed to parse data. Please check your input.".into(),
            ...
            // Internal
            TorError::Internal(_) => 
                "An internal error occurred. Please report this bug.".into(),
            TorError::NotBootstrapped => 
    ...
    pub fn circuit_destroyed(reason: u8) -> Self {
        ...
        TorError::CircuitDestroyed { reason, reason_name }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_error_codes() {
        assert_eq!(TorError::ConnectionFailed("test".into()).code(), ErrorCode::ConnectionFailed);
        assert_eq!(TorError::Timeout.code(), ErrorCode::ConnectionTimeout);
        assert_eq!(TorError::CertificateError("test".into()).code(), ErrorCode::CertificateError);
    }
    
    #[test]
    fn test_circuit_destroyed() {
        let err = TorError::circuit_destroyed(1);
        if let TorError::CircuitDestroyed { reason, reason_name } = err {
            assert_eq!(reason, 1);
