...
pub fn apply() -> Result<(), JsValue> {
    ...
    let replacement = Closure::wrap(
        Box::new(|| -> JsValue { JsValue::from_f64(0.0) }) as Box<dyn FnMut() -> JsValue>
    );
    Reflect::set(
        &date_proto,
        &JsValue::from_str("getTimezoneOffset"),
        replacement.as_ref(),
    )?;
    replacement.forget();
        ...
        let apply_trap = Closure::wrap(Box::new(
            move |_target: JsValue, this_arg: JsValue, args: JsValue| -> Result<JsValue, JsValue> {
                ...
                let options = if args_arr.length() > 1 && args_arr.get(1).is_object() {
                    ...
                    let obj =
                        js_sys::eval(&"Object.assign({}, {}, {timeZone: 'UTC'})".to_string())?
                            .unchecked_into::<Function>();
                    Reflect::apply(
                        &obj,
                        &JsValue::UNDEFINED,
                        &Array::of2(&src, &JsValue::UNDEFINED),
                    )?;
                    // Simpler: use Object.assign
                    let assign: Function =
                        js_sys::eval("(src) => Object.assign({}, src, {timeZone: 'UTC'})")?
                            .unchecked_into();
    ...
    let replacement = Closure::wrap(Box::new(
        move |_target: JsValue, this_arg: JsValue, _args: JsValue| -> Result<JsValue, JsValue> {
            let to_utc: Function =
                Reflect::get(&this_arg, &JsValue::from_str("toUTCString"))?.unchecked_into();
            let utc_str = Reflect::apply(&to_utc, &this_arg, &Array::new())?;
            let s = utc_str.as_string().unwrap_or_default();
            Ok(JsValue::from_str(
                &s.replace("GMT", "GMT+0000 (Coordinated Universal Time)"),
            ))
        },
    )
        as Box<dyn FnMut(JsValue, JsValue, JsValue) -> Result<JsValue, JsValue>>);
    ...
    if let Ok(intl) = intl {
        if !intl.is_undefined() {
            ...
            if !dtf.is_undefined() {
                ...
                let orig_resolved =
                    Reflect::get(&dtf_proto, &JsValue::from_str("resolvedOptions"))?;
