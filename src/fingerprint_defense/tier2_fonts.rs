...
fn is_standard_font(family: &str) -> bool {
    STANDARD_FONTS.iter().any(|f| f.eq_ignore_ascii_case(family))
}
...
fn apply_measure_text() -> Result<(), JsValue> {
    ...
    let apply_trap = Closure::wrap(Box::new(move |_target: JsValue, this_arg: JsValue, args: JsValue| -> Result<JsValue, JsValue> {
        ...
        if let Some(font_str) = font.as_string() {
            let families: Vec<&str> = font_str.split(',').map(|f| f.trim().trim_matches(|c| c == '"' || c == '\'').trim()).collect();
            ...
            if !all_standard {
                ...
                Reflect::set(&this_arg, &JsValue::from_str("font"), &JsValue::from_str("16px monospace"))?;
                let fallback = proxy_helpers::call_function(&orig_fn, &this_arg, &args)?;
                Reflect::set(&this_arg, &JsValue::from_str("font"), &JsValue::from_str(&saved_font))?;
...
fn extract_font_family(spec: &str) -> String {
    ...
    if let Some(last) = spec.rsplit(',').next() {
        let trimmed = last.trim().trim_matches(|c: char| c == '"' || c == '\'').trim();
        // Skip the size/weight prefix if present
        if let Some(family) = trimmed.rsplit(' ').next() {
            return family.trim_matches(|c: char| c == '"' || c == '\'').to_string();
        }
