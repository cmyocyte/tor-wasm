...
fn apply_element_rects() -> Result<(), JsValue> {
    ...
    Reflect::set(&proto, &JsValue::from_str("getBoundingClientRect"), &proxied)?;
...
fn round_dom_rect(rect: &JsValue) -> Result<JsValue, JsValue> {
    ...
    let x = Reflect::get(rect, &JsValue::from_str("x"))?.as_f64().unwrap_or(0.0).round();
    let y = Reflect::get(rect, &JsValue::from_str("y"))?.as_f64().unwrap_or(0.0).round();
    let w = Reflect::get(rect, &JsValue::from_str("width"))?.as_f64().unwrap_or(0.0).round();
    let h = Reflect::get(rect, &JsValue::from_str("height"))?.as_f64().unwrap_or(0.0).round();

    Reflect::construct(&dom_rect_ctor, &Array::of4(
        &JsValue::from_f64(x), &JsValue::from_f64(y),
        &JsValue::from_f64(w), &JsValue::from_f64(h),
    ))
}
