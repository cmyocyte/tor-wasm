...
use js_sys::{Array, Function, Reflect, Uint8Array, Uint8ClampedArray};
    ...
    let apply_trap = Closure::wrap(Box::new(
        move |_target: JsValue, this_arg: JsValue, args: JsValue| -> Result<JsValue, JsValue> {
            ...
            let get_ctx: Function =
                Reflect::get(&this_arg, &JsValue::from_str("getContext"))?.unchecked_into();
            let ctx = Reflect::apply(&get_ctx, &this_arg, &Array::of1(&JsValue::from_str("2d")))?;
            if !ctx.is_null() && !ctx.is_undefined() {
                let width = Reflect::get(&this_arg, &JsValue::from_str("width"))?
                    .as_f64()
                    .unwrap_or(0.0);
                let height = Reflect::get(&this_arg, &JsValue::from_str("height"))?
                    .as_f64()
                    .unwrap_or(0.0);
                if width > 0.0 && height > 0.0 {
                    // getImageData triggers perturbation, putImageData writes back
                    let get_id: Function =
                        Reflect::get(&ctx, &JsValue::from_str("getImageData"))?.unchecked_into();
                    let img_data = Reflect::apply(
                        &get_id,
                        &ctx,
                        &Array::of4(
                    ...
                    let put_id: Function =
                        Reflect::get(&ctx, &JsValue::from_str("putImageData"))?.unchecked_into();
                    let _ = Reflect::apply(
                        &put_id,
                        &ctx,
                        &Array::of3(&img_data, &JsValue::from_f64(0.0), &JsValue::from_f64(0.0)),
                    );
                }
            }
