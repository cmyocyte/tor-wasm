...
pub enum VerifyError {
    ...
    LowBandwidth { fingerprint: String, claimed: u64, observed: u64 },
...
impl std::fmt::Display for VerifyError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            VerifyError::LowBandwidth { fingerprint, claimed, observed } => 
                write!(f, "Relay {} claimed {}b/s but observed {}b/s", fingerprint, claimed, observed),
            VerifyError::DenyListed { fingerprint, reason } => 
...
impl RelayVerifier {
    ...
    pub fn deny_relay(&mut self, fingerprint: &str, reason: &str) {
        self.deny_list.insert(fingerprint.to_string(), reason.to_string());
        log::warn!("🚫 Relay {} added to deny list: {}", &fingerprint[..8.min(fingerprint.len())], reason);
    }
    ...
    pub fn record_bandwidth(&mut self, fingerprint: &str, bytes: u64, duration_ms: u64, claimed: u64) {
        let observation = self.bandwidth_observations
    ...
    pub fn stats(&self) -> RelayVerifierStats {
        let suspicious_count = self.bandwidth_observations.values()
...
mod tests {
    ...
    fn test_deny_list() {
        ...
        if let Err(VerifyError::DenyListed { fingerprint, reason }) = result {
