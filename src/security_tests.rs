...
use crate::protocol::{CertsCell, CircuitKeys, Ed25519Certificate, NtorHandshake};
...
"#;
        ...
        assert!(
            result.is_err(),
            "Should reject consensus with only 2 signatures"
        );

        if let Err(TorError::ConsensusError(msg)) = result {
            assert!(
                msg.contains("need"),
                "Error should mention required signatures"
            );
        }
    }
...
mod certificate_security {
    ...
    fn test_certs_cell_rejects_truncated() {
        ...
        let data = [
            1u8, 0x01, 0x00, 0x64, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ];
        let result = CertsCell::parse(&data);
        assert!(result.is_err());
    }
